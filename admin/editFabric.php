...
if ($result->num_rows > 0) {
    ...
    echo "<script>alert('No fabric found with the provided ID.');</script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle image upload
    $image_dir = "D:/xampp/htdocs/ab/fabric/"; // Directory where the images are stored
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Fabric updated successfully.');</script>";
    } else {
        echo "<script>alert('Error updating fabric: " . $conn->error . "');</script>";
    }
}
